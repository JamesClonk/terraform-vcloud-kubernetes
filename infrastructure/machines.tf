...
resource "vcd_vapp_vm" "k8s_bastion" {
  ...
  customization {
    ...
    sed 's/127\.0\.1\.1.*/127.0.1.1 ${var.k8s_cluster_name}-bastion/g' -i /etc/hosts || true
    sed 's/${cidrhost(var.k8s_cidr, 20)}.*/${cidrhost(var.k8s_cidr, 20)} ${var.k8s_cluster_name}-bastion/g' -i /etc/hosts || true
...
#     sed 's/127\.0\.1\.1.*/127.0.1.1 ${var.k8s_cluster_name}-master-${count.index}/g' -i /etc/hosts || true
#     sed 's/${cidrhost(var.k8s_cidr, 50 + count.index)}.*/${cidrhost(var.k8s_cidr, 50 + count.index)} ${var.k8s_cluster_name}-master-${count.index}/g' -i /etc/hosts || true
...
#     sed 's/127\.0\.1\.1.*/127.0.1.1 ${var.k8s_cluster_name}-worker-${count.index}/g' -i /etc/hosts || true
#     sed 's/${cidrhost(var.k8s_cidr, 100 + count.index)}.*/${cidrhost(var.k8s_cidr, 100 + count.index)} ${var.k8s_cluster_name}-worker-${count.index}/g' -i /etc/hosts || true
