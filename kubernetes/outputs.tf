resource "local_sensitive_file" "kubeconfig_file" {
  filename        = "kubeconfig"
  content         = replace(module.k3s.kube_config, cidrhost(var.k8s_node_cidr, 50), var.loadbalancer_ip)
  file_permission = "0600"
}

output "cluster_api_endpoint" {
  value = replace(module.k3s.kubernetes.api_endpoint, cidrhost(var.k8s_node_cidr, 50), var.loadbalancer_ip)
}
